        ...
        try:
            ...
            if("type" in peer_info):
                if(peer_info["type"] == "uploadPing"):
                    ...
                    self.logger.debug(f"FILE REQUEST FOR FILE {peer_info['fileID'][0]}")
                    
                    #Add file to request list   
                    self.AddFileToRequestList(peer_info['fileID'][0])
            else:
        ...
        try:
            ...
            for chunkLocation in chunkLocations:
                ...
                if(user != None):
                    ...
                    self.RequestChunkFromUser(user, fileID, chunkIndex)
                        
        except Exception as e:
            print(f"Error {e} with fileID {fileID} in RequestFileFromUser")

    def RequestChunkFromUser(self, targetUserCode, fileID, chunkIndex): #TODO
        try:
